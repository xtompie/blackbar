...
class Redactor:
    ...
    async def redact(self, text: str) -> tuple[str, Counter[str], Counter[str], list[tuple[str, str]]]:
        ...
        The keys identify vault entries, so `watch` can show exactly which values this
        request replaced without the daemon ever shipping the values themselves.
        """
        if not text.strip():
        ...
        if cached is None:
            ...
            hits = tuple(
                (s.kind, s.layer, _key_of(self.vault.mask(s.kind, s.text))) for s in spans
            )
